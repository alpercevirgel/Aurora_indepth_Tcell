...
table_age <- gating_meta %>%
  ...
  summarise(
    Total_Samples = n(),
    CMV_Positive_Count = sum(MIA_CMV_Seropositivity == "1", na.rm = TRUE),
    Male_Count = sum(sex == "1", na.rm = TRUE),
    MIA_CMV_Seropositivity_Percentage = mean(MIA_CMV_Seropositivity == "1", na.rm = TRUE) * 100,
...
table_age_total <- gating_meta %>%
  ...
  select(age_group, Total_Samples, CMV_Positive_Count, MIA_CMV_Seropositivity_Percentage,
         Male_Count, Male_Percentage, Age_Range)

table_agegroups <- bind_rows(table_age, table_age_total)
table_agegroups
...
table_vac_all <- trivac_resp_quartiles %>%
  ...
  mutate(
    ...
    MIA_CMV_Seropositivity_Percentage = round(MIA_CMV_Seropositivity_Percentage, 0),
    Male_Percentage = round(Male_Percentage, 0),
    ResponseGroup_Triple2 = "all"
  ) %>%
  select(ResponseGroup_Triple2, Total_Samples, CMV_Positive_Count, MIA_CMV_Seropositivity_Percentage,
         Male_Count, Male_Percentage, Age_Range)

table_vac <- bind_rows(table_vac, table_vac_all)
write_xlsx(table_vac, paste0(table_save.dir, "table_vac.xlsx"))
...
## TQ table with directions
```{r}
# Calculate median values for each group
median_values <- df_stat %>%
  group_by(variable, ResponseGroup_Triple2) %>%
  summarise(Median_Value = median(value, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = ResponseGroup_Triple2, values_from = Median_Value)

# Merge median values with Wilcoxon results
wilcox_TQ_som_p <- wilcox_TQ_som_p %>%
  left_join(median_values, by = "variable") %>%
  mutate(
    Direction = case_when(
      estimate > 0 ~ paste("Higher in", group1),
      estimate < 0 ~ paste("Higher in", group2),
      TRUE ~ "No Difference"
    )
  ) %>%
  select(variable, group1, group2, p, p.adj, Direction)

# Save results to Excel
write_xlsx(wilcox_TQ_som_p, path = paste0(table_save.dir, "SOM_TQ_p.xlsx"))
```
